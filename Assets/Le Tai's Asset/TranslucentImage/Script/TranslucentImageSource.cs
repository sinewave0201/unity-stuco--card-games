...
#region Properties
    ...
    void StartCull()
    {
        ...
        {
            var provider = activeRegionProviders[i];
            if (!provider.HaveActiveRegion())
                continue;
            ...
            provider.GetActiveRegion(vpMatrixCache, out var activeRegion);
        ...
        if (haveAnyActiveRegion)
        {
            ...
            {
                activeRegionJobResult[0] = ActiveRegion = FULLSCREEN_REGION;
            }
        }
        else
        {
            // If not haveAnyActiveRegion, should skip blurring in XR, so use SkipCulling instead of ShouldCull
            activeRegionJobResult[0] = ActiveRegion = SkipCulling ? FULLSCREEN_REGION : Rect.zero;
        }
    ...
    public bool CompleteCull()
    {
        ...
        if (MaxUpdateRate < float.PositiveInfinity
         && ActiveRegion.width > minSize
         && ActiveRegion.height > minSize)
        {
        ...
        return ActiveRegion.width > minSize && ActiveRegion.height > minSize;
    }
    ...
    void CreateNewBlurredScreen(Vector2Int camPixelSize)
    {
        ...
        BlurredScreen.autoGenerateMips = false;

#if LETAI_PARAFORM
        // TODO: Make this configurable
        // Why do mipmaps cause edge artifact? May need to re-enable edge dimming if use mips
        // Can this be optimized?
        // BlurredScreen.useMipMap        = true;
        // BlurredScreen.autoGenerateMips = true;
        // BlurredScreen.filterMode       = FilterMode.Trilinear;
#endif
